...
if ( !class_exists('Plethora_Module_Navwalker') ) {
	...
	class Plethora_Module_Navwalker extends Walker_Nav_Menu {
		...
		public static $feature_title         = "NavWalker Menu Module";												// Feature display title  (string)
		public static $feature_description   = "Integration module for native WP Walker navigation functionality";	// Feature display description (string)
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
				...
				if ( is_object($args) && $args->has_children && $depth === 0)
					$class_names .= ' lihasdropdown';

				if ( is_object($args) && $args->has_children && $depth > 0 )
					$class_names .= ' sublihasdropdown';
				...
				*/
				...
				foreach ( $atts as $attr => $value ) {
					if ( ! empty( $value ) ) {
						...
						if ( !( $item->object === 'wpml_ls_menu_item' && $attr === 'title' ) ) {
				...
				if ( is_object($args) ) { 
					...
					$output = '<li><a href="'. esc_url( admin_url('nav-menus.php') ) .'">'. esc_html__( "Please Create a Menu", 'plethora-framework' ) . '</a>';
				}
			}
		}
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				$fb_output .= '>';
				$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">'. esc_html__( "Please Create a Menu", 'plethora-framework' ) . '</a></li>';
				$fb_output .= '</ul>';
